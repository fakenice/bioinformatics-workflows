    ...
    sources: [
      {
        ...
        toolRows: [
          { stepIndex: 0, name: "PLINK", version: "1.9 / 2.0", params: "--mind 0.03 --geno 0.02 --maf 0.01 --hwe 1e-6", docker: "", notes: "样本缺失率 < 3%，变异缺失率 < 2%，MAF > 1%，HWE P > 1e-6。" },
          { stepIndex: 0, name: "PLINK --king", version: "2.0", params: "--king-cutoff 0.0884", docker: "", notes: "移除二级以上亲属（PI_HAT < 0.0884）。" },
          { stepIndex: 1, name: "GCTA / PLINK 2.0", version: "1.94 / 2.0", params: "--pca 20", docker: "", notes: "PCA 计算前 20 个主成分。检查 PC1 vs PC2 是否有群体异常样本。" },
          { stepIndex: 2, name: "PLINK --logistic", version: "1.9", params: "--covar PC1-PC5 --ci 0.95", docker: "", notes: "基础逻辑回归，适合中小样本。" },
          { stepIndex: 2, name: "SAIGE", version: "1.3+", params: "step1: fitNULLGLMM, step2: SPAtests", docker: "", notes: "推荐用于大样本 + 病例不平衡。使用 SPA 校正。" },
          { stepIndex: 2, name: "REGENIE", version: "3.4+", params: "--step 1 --step 2 --bt", docker: "", notes: "两步法，适合 UKB 级别大数据。低内存模式。" },
          { stepIndex: 3, name: "LDSC / GCTA", version: "1.0.1 / 1.94", params: "", docker: "", notes: "计算 λ_GC（目标 0.95-1.05）。Manhattan/QQ 图用 R qqman 包。" },
        ],
        references: [
    ...
    sources: [
      {
        ...
        toolRows: [
          { stepIndex: 0, name: "TwoSampleMR R 包", version: "0.6+", params: "extract_instruments(p1=5e-8, clump=T, r2=0.001, kb=10000)", docker: "", notes: "F-statistic = beta²/se²，要求 F > 10。" },
          { stepIndex: 1, name: "TwoSampleMR harmonise_data()", version: "0.6+", params: "action=2", docker: "", notes: "action=2 去除回文 SNP（MAF > 0.4）。" },
          { stepIndex: 2, name: "TwoSampleMR mr()", version: "0.6+", params: "method_list=c('mr_ivw','mr_egger_regression','mr_weighted_median')", docker: "", notes: "IVW 为主方法。MR-Egger 检验水平多效性。" },
          { stepIndex: 3, name: "TwoSampleMR mr_heterogeneity()", version: "0.6+", params: "", docker: "", notes: "Cochran's Q P < 0.05 表示异质性。" },
          { stepIndex: 3, name: "TwoSampleMR mr_pleiotropy_test()", version: "0.6+", params: "", docker: "", notes: "MR-Egger 截距 P < 0.05 表示水平多效性。" },
          { stepIndex: 3, name: "MR-PRESSO", version: "1.0", params: "NbDistribution=1000, SignifThreshold=0.05", docker: "", notes: "离群值检测 + 失真检验。" },
          { stepIndex: 3, name: "TwoSampleMR mr_leaveoneout()", version: "0.6+", params: "", docker: "", notes: "逐个 SNP 剔除，检查是否有单一 SNP 驱动结果。" },
          { stepIndex: 4, name: "TwoSampleMR mv_multivariable_mr()", version: "0.6+", params: "", docker: "", notes: "多变量 MR 控制混杂暴露。" },
        ],
        references: [
    ...
    sources: [
      {
        ...
        toolRows: [
          { stepIndex: 0, name: "PLINK", version: "1.9 / 2.0", params: "--bfile", docker: "", notes: "Target data 需为 PLINK 格式（bed/bim/fam）。" },
          { stepIndex: 1, name: "PLINK / QCTOOL", version: "2.0 / 2", params: "--maf 0.01 --info 0.8", docker: "", notes: "Base data 过滤：MAF > 0.01, INFO > 0.8。" },
          { stepIndex: 2, name: "PRSice-2", version: "2.3+", params: "--bar-levels 5e-8,1e-6,...,1 --fastscore", docker: "", notes: "C+T 方法。--fastscore 只计算指定阈值。" },
          { stepIndex: 2, name: "LDpred2 (bigsnpr)", version: "1.12+", params: "snp_ldpred2_auto()", docker: "", notes: "Bayesian 自动调参，推荐。需要 LD 参考面板。" },
          { stepIndex: 2, name: "PRS-CS", version: "1.0", params: "--n_gwas=<N>", docker: "", notes: "Bayesian 连续收缩，大样本推荐。PRS-CS-auto 自动调参。" },
          { stepIndex: 3, name: "R / PLINK", version: "4.3+ / 2.0", params: "--score", docker: "", notes: "评估指标：R²（连续）、AUC（二分类）、OR per SD。" },
        ],
        references: [
    ...
    sources: [
      {
        ...
        toolRows: [
          { stepIndex: 0, name: "GATK / BCFtools", version: "4.5 / 1.17", params: "QUAL>30, DP>10, MAF<0.01", docker: "", notes: "QUAL > 30, DP > 10, MAF < 1%。仅保留功能性变异（missense/nonsense/splice-site）。CADD > 20 进一步过滤。" },
          { stepIndex: 1, name: "VEP / ANNOVAR", version: "112 / 2023", params: "--everything", docker: "ensemblorg/ensembl-vep:112", notes: "按基因分组变异。通常排除 synonymous。可选按功能域分层（如 LOFTEE、REVEL > 0.5）。" },
          { stepIndex: 2, name: "SKAT R 包", version: "2.2+", params: "out_type='D' or 'C'", docker: "", notes: "SKAT_Null_Model(y ~ PC1+PC2+Age+Sex, out_type='D')。out_type='D' 二分类，'C' 连续。" },
          { stepIndex: 3, name: "SKAT Burden()", version: "2.2+", params: "r.corr=1", docker: "", notes: "假设基因内所有罕见变异方向一致（同向效应时效力最高）。方向不一致时效力显著下降。" },
          { stepIndex: 4, name: "SKAT SKAT()", version: "2.2+", params: "r.corr=0", docker: "", notes: "不假设变异方向一致，对双向效应（既有保护性又有致病性变异）效力更高。" },
          { stepIndex: 5, name: "SKAT SKAT()", version: "2.2+", params: "r.corr=NA", docker: "", notes: "SKAT-O 自动选择最优 r.corr 参数，推荐作为默认方法。" },
          { stepIndex: 6, name: "R p.adjust()", version: "4.3+", params: "method='BH'", docker: "", notes: "FDR 校正，显著性阈值 q < 0.05。" },
        ],
        references: [
