...
const PIPELINES: PipelineTemplate[] = [
    ...
    version: "1.0.0",
    overview:
      "适用于父母-子代 Trio 家系的全基因组种系变异联合检测。基于 GATK Best Practices (v4.5)，覆盖 FASTQ→VCF 全链路，包含联合分型、de novo 检出、遗传模式筛选。",
    overviewEn:
      "Joint germline variant detection for parent-child trio families using GATK Best Practices (v4.5). Covers the full FASTQ-to-VCF pipeline including joint genotyping, de novo detection, and inheritance pattern filtering.",
    icon: "dna",
    sources: [
      {
        id: "gatk",
        name: "GATK Best Practices",
        nameEn: "GATK Best Practices",
        type: "official",
        ...
        stepNamesEn: [
          ...
          "GVCF Consolidation",
          "Joint Genotyping",
          "VQSR Filtering / Hard-Filtering",
          "De Novo Detection",
          "Inheritance Pattern Screening",
          "Variant Annotation + QC Report",
        ],
        stepDescs: [
        ...
        stepDescsEn: [
          "Align Clean Reads to the hg38 reference genome",
          "Mark PCR duplicates and recalibrate base quality scores",
          "HaplotypeCaller in GVCF mode, one per sample",
          "Import 3 GVCFs into GenomicsDB",
          "Joint genotyping from GenomicsDB for trio",
          "VQSR recalibration for SNPs and Indels; Hard-Filtering for small cohorts",
          "Identify de novo variants using PED pedigree file",
          "Filter by AD/AR/X-linked inheritance patterns plus phenotype",
          "Functional annotation + MultiQC summary report",
        ],
        toolRows: [
          { stepIndex: 0, name: "BWA-MEM2", version: "2.2", params: "-M -R @RG\\\tID:sample\\\tSM:sample\\\tPL:ILLUMINA", docker: "biocontainers/bwa-mem2:2.2.1", notes: "BWA-MEM2 比 BWA-MEM 快约 2×。也可选 DragMap 提速 3-5×。", notesEn: "BWA-MEM2 is ~2× faster than BWA-MEM. DragMap option available for 3-5× speedup." },
          { stepIndex: 1, name: "GATK MarkDuplicates", version: "4.5", params: "", docker: "broadinstitute/gatk:4.5.0.0", notes: "BQSR 基于 dbSNP/1000G/Mills 等已知位点集。家系分析中不推荐完全移除重复，仅标记即可。", notesEn: "BQSR based on dbSNP/1000G/Mills known variant sets. For trio analysis, marking duplicates is preferred over removal." },
          ...
          { stepIndex: 2, name: "GATK HaplotypeCaller", version: "4.5", params: "-ERC GVCF --native-pair-hmm-threads 8", docker: "broadinstitute/gatk:4.5.0.0", notes: "GVCF 模式便于后续增量联合 calling。Trio 中 3 个样本完全并行。", notesEn: "GVCF mode enables incremental joint calling. 3 trio samples run fully in parallel." },
          { stepIndex: 3, name: "GATK GenomicsDBImport", version: "4.5", params: "--genomicsdb-workspace-path gendb --intervals chr.list", docker: "broadinstitute/gatk:4.5.0.0", notes: "按染色体区间并行导入。3 个样本的 Trio 直接全基因组导入即可。", notesEn: "Parallel import by chromosome interval. For a 3-sample trio, direct genome-wide import suffices." },
          { stepIndex: 4, name: "GATK GenotypeGVCFs", version: "4.5", params: "--include-non-variant-sites", docker: "broadinstitute/gatk:4.5.0.0", notes: "联合分型可利用群体信息提高稀有变异敏感性，并输出一致的基因型矩阵。对 Trio 设计增益明显。", notesEn: "Leverages population information to improve rare variant sensitivity and produce a consistent genotype matrix. Significant boost for trio designs." },
          { stepIndex: 5, name: "GATK VariantRecalibrator", version: "4.5", params: "-mode SNP --max-gaussians 4", docker: "broadinstitute/gatk:4.5.0.0", notes: "⚠ VQSR 至少需要 30 样本才稳健。Trio 小样本建议 Hard-Filtering：QD<2.0, FS>60, MQ<40, SOR>3.0。", notesEn: "VQSR requires >=30 samples for robustness. For small trio cohorts, Hard-Filtering recommended: QD<2.0, FS>60, MQ<40, SOR>3.0." },
          ...
          { stepIndex: 6, name: "GATK PossibleDeNovo", version: "4.5", params: "--ped family.ped --max-parent-af 0.001", docker: "broadinstitute/gatk:4.5.0.0", notes: "筛选仅先证者存在、父母为参考纯合的变异。结合 gnomAD MAF<0.01 及功能注释进一步缩窄候选。", notesEn: "Filter for variants present only in proband, homozygous reference in both parents. Narrow candidates with gnomAD MAF<0.01 and functional annotation." },
          { stepIndex: 7, name: "slivar", version: "0.3", params: "--ped family.ped --trio --gnomad-max-af 0.01 --impactful", docker: "brentp/slivar:0.3", notes: "slivar 一站式支持 de novo、compound het、AR homozygous、AD inherited 等模式。结合表型 HPO 词条效果更佳。", notesEn: "slivar provides one-stop support for de novo, compound het, AR homozygous, AD inherited patterns. Combine with HPO terms for better results." },
          ...
          { stepIndex: 8, name: "Ensembl VEP", version: "112", params: "--everything --fork 8 --pick", docker: "ensemblorg/ensembl-vep:112", notes: "VEP 输出含 gnomAD AF、ClinVar 临床意义、CADD/SIFT/PolyPhen 预测分数。MultiQC 汇总覆盖度、Ti/Tv、比对率等。", notesEn: "VEP output includes gnomAD AF, ClinVar significance, CADD/SIFT/PolyPhen scores. MultiQC summarizes coverage, Ti/Tv ratio, alignment rate, etc." },
